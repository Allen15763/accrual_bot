...
from accrual_bot.tasks.config import ENTITY_CONFIG
...
class UnifiedPipelineService:
    ...
    def build_pipeline(
        ...
        elif proc_type == 'VARIANCE' and entity == 'SCT':
            return orchestrator.build_variance_pipeline(enriched_file_paths)
        else:
    ...
    def _enrich_file_paths(
        ...
        try:
            ...
            logger.error(f"Failed to enrich file_paths: {e}", exc_info=True)
