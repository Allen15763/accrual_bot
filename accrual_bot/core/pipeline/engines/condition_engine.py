...
從 TOML 配置讀取規則，動態建構 pandas boolean mask 並依序應用狀態標籤。
各 entity 透過 entity_type 參數區分配置來源（如 fa_accounts.spx / fa_accounts.sct）。

支援規則類型範例：
- sct_erm_status_rules: SCT ERM 邏輯狀態判斷
- spx_status_stage1_rules: SPX 第一階段狀態標籤
- spx_erm_status_rules: SPX ERM 邏輯狀態判斷

設計原則：
- 引擎本身不含任何 entity 特定邏輯，所有業務規則由 TOML 配置驅動
- entity_type 僅用於解析 entity 相關的配置引用（如 fa_accounts）
"""
...
class ConditionEngine:
    ...
    從 TOML 配置讀取條件規則，動態建構 pandas boolean mask 並應用狀態。
    每個規則包含 priority、status_value、note、combine 和 checks 列表。

    Usage:
        engine = ConditionEngine('sct_erm_status_rules', entity_type='SCT')
        df, stats = engine.apply_rules(df, 'PO狀態', context)
    """

    def __init__(self, config_section: str, entity_type: str = 'SPX'):
        """
        初始化引擎

        Args:
            config_section: TOML 配置區段名稱
                           如 'sct_erm_status_rules' 或 'spx_erm_status_rules'
            entity_type: Entity 類型（'SPX', 'SCT', 'SPT'），用於解析 entity 相關配置
        """
        self.config_section = config_section
        self.entity_type = entity_type
        self.rules = self._load_rules()
        logger.info(f"ConditionEngine 已載入 {len(self.rules)} 條規則 "
                    f"(來源: {config_section}, entity: {entity_type})")
    ...
    def _evaluate_check(
        ...
        # === 預先計算的 mask（從 context 取得）；no_status不能透過prebuilt mask取得===
        if check_type in prebuilt and check_type != 'no_status':
        ...
        # === FA 類（使用 entity_type 解析配置）===
        if check_type == 'is_fa':
            if 'is_fa' in prebuilt:
                return prebuilt['is_fa']
            fa_accounts = config_manager._config_toml.get(
                'fa_accounts', {}
            ).get(self.entity_type.lower())
            if fa_accounts is None:
                logger.warning(f"未找到 fa_accounts.{self.entity_type.lower()} 配置")
                return None
    ...
    def _resolve_ref(self, key: str) -> Any:
        ...
        """
        # 特殊處理：asset_suppliers = kiosk + locker（SPX 特有的合併邏輯）
