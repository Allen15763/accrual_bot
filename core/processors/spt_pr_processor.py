...
繼承BasePRProcessor，實現SPT特有的PR處理邏輯
"""
...
try:
    ...
    from ...utils.config import REF_PATH_SPT
...
class SptPRProcessor(BasePRProcessor):
    """SPT PR專用處理器"""
    
    def __init__(self, entity_type: str = "SPT"):
        super().__init__(entity_type)
        self.logger = Logger().get_logger(f"{__name__}.SptPRProcessor")
    ...
    def process(self, raw_data_file: str, filename: str,
        ...
        """
        self.logger.info(f"開始SPT PR處理: {filename}")
        ...
        try:
            ...
            self.logger.error(f"SPT PR處理失敗: {e}")
            return ProcessingResult(
                success=False,
                message=f"SPT PR處理失敗: {str(e)}"
            )
    
    def _spt_preprocess_data(self, raw_data_file: str) -> Tuple[pd.DataFrame, str]:
        """SPT特有的前置數據處理"""
        self.logger.debug("執行SPT PR前置處理")
        
        # 載入數據
        df, yyyymm = self.import_raw_data(raw_data_file)
        
        # SPT特有的前置處理邏輯
        # df = self._apply_spt_pr_data_cleaning(df)
        # df = self._apply_spt_pr_validation_rules(df)
        
        return df, yyyymm
    
    def _spt_postprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        """SPT特有的後置數據處理"""
        self.logger.debug("執行SPT PR後置處理")
        
        # SPT特有的後置處理邏輯
        # df = self._apply_spt_pr_business_rules(df)
        # df = self._apply_spt_pr_formatting(df)
        
        return df
    
    def _apply_spt_pr_data_cleaning(self, df: pd.DataFrame) -> pd.DataFrame:
        """SPT PR特有的數據清理"""
        # SPT特有：處理PR請購單號格式
        if '請購單號' in df.columns:
            df['請購單號'] = df['請購單號'].astype(str).str.upper().str.strip()
            # SPT特殊規則：請購單號必須以SPT開頭
            mask = ~df['請購單號'].str.startswith('SPT')
            df.loc[mask, '請購單號'] = 'SPT' + df.loc[mask, '請購單號']
        
        return df
    
    def _apply_spt_pr_validation_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        """SPT PR特有的驗證規則"""
        # SPT特有：PR金額驗證（與MOB不同）
        if '金額' in df.columns:
            # SPT規定：單筆PR金額不能超過1000萬
            df.loc[df['金額'] > 10000000, '備註'] = df['備註'].fillna('') + ' [SPT PR金額超限]'
        
        return df
    
    def _apply_spt_pr_business_rules(self, df: pd.DataFrame) -> pd.DataFrame:
        """SPT PR特有的業務規則"""
        # SPT特有：預算中心驗證
        if '預算中心' in df.columns:
            # SPT特殊規則：某些預算中心需要額外審核
            special_cost_centers = ['CC001', 'CC002', 'CC999']
            mask = df['預算中心'].isin(special_cost_centers)
            df.loc[mask, '審核標記'] = 'SPT_BUDGET_REVIEW'
        
        return df
    
    def _apply_spt_pr_formatting(self, df: pd.DataFrame) -> pd.DataFrame:
        """SPT PR特有的格式化"""
        # SPT特有的欄位順序
        spt_pr_column_order = [
            '公司', '請購單號', '申請人', '部門', '預算中心',
        ...
        # 重新排列欄位
        existing_columns = [col for col in spt_pr_column_order if col in df.columns]
        other_columns = [col for col in df.columns if col not in spt_pr_column_order]
    ...
    def import_reference_data(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        ...
        """
        try:
            url = self.config_manager._config_data.get('PATHS').get('ref_path_spt', REF_PATH_SPT)
