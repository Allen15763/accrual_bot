...
### 4.3 `PipelineWithCheckpoint` 繞過 `__call__` 包裝器

這是整個模組中最值得關注的一個實作細節：

```python
# checkpoint.py:439 — 直接呼叫 execute()，而非 __call__()
...
`PipelineWithCheckpoint.execute_with_checkpoint()` 直接呼叫 `step.execute(context)`，**完全繞過了 `PipelineStep.__call__()` 提供的**：
...
**影響**：任何使用 `execute_pipeline_with_checkpoint()` 的流程（包括 Streamlit UI 和 CLI 的 checkpoint 路徑）都無法享受 retry 保護，若某步驟可能因瞬時錯誤失敗（如網路請求），需手動在步驟內部實現重試。
...
#### ❌ `PipelineWithCheckpoint` 繞過 `__call__` 包裝器（中等嚴重度）

如 [4.3 節](#43-pipelinewithcheckpoint-繞過-__call__-包裝器) 所述，checkpoint 路徑的執行無法享受 retry 和 timeout 保護。
...
| 問題 | 嚴重度 | 位置 | 說明 |
|------|--------|------|------|
| `PipelineWithCheckpoint` 繞過 `__call__` | 中 | `checkpoint.py:439` | Retry/Timeout 失效 |
