...
> **範圍**：`accrual_bot/core/pipeline/` — 29 個 Python 檔案，~6,277 行（含 `engines/` 子目錄）
...
Accrual Bot 是一套每月執行的財務應計（Accrual）自動化系統，負責處理 SPT、SPX、SCT 三個業務實體的採購訂單（PO）與採購申請（PR）資料，產出可供會計師稽核的底稿。
...
Phase 6 (Mar 2026):  15 個向後兼容 re-export 檔案（spt_*.py / spx_*.py）建立，完成遷移至 tasks/ 目錄
```
...
| `steps/spt_*.py`, `steps/spx_*.py` × 15 | ~76 | 向後兼容重新導出（直接 re-export 至 `tasks/` 目錄，非獨立 shim 檔案） |
| `engines/__init__.py` | — | 條件引擎子模組初始化 |
| `engines/condition_engine.py` | 560 | 配置驅動條件引擎（SPX/SCT 使用） |
...
**效益**：消除了 SPT/SPX 載入步驟中約 750 行的重複代碼，且新增實體（如已完成的 SCT）只需實現 4 個鉤子方法。
...
│   ├── SCTDataLoadingStep    (tasks/sct/steps/)
│   └── SCTPRDataLoadingStep  (tasks/sct/steps/)
