...
#### ❌ `ClosingListIntegrationStep` 中有硬編碼的 Google Sheets ID
...
#### ❌ `ColumnAdditionStep` 依賴 context variable 偵測 PO/PR
...
#### ❌ `spx_evaluation_2.py` 的模組說明與位置不一致
...
#### ❌ `PPEDataLoadingStep._load_renewal_list()` 混用同步與非同步
...
### 7.2 `ClosingListIntegrationStep` 的配置化改造
