...
抽取了 SPT 和 SPX 載入步驟中重複的約 400–500 行程式碼，形成 `BaseLoadingStep` 模板基類。然而，觀察現有的 `spt_loading.py`，`SPTDataLoadingStep` 和 `SPTPRDataLoadingStep` **並未繼承** `BaseLoadingStep`，仍直接繼承 `PipelineStep`，表示這項重構對 SPT 主要載入步驟的影響有限，只有採購子模組的載入步驟（`spt_procurement_loading.py`）才使用了 `BaseLoadingStep`。
...
**位置**：`spt_loading.py`，`SPTDataLoadingStep._load_ap_invoice()`
...
#### BUG-2：`_rearrange_reviewer_col` 缺少防護

**位置**：`spt_steps.py`，`SPTPostProcessingStep._rearrange_reviewer_col()`

```python
def _rearrange_reviewer_col(self, df):
    a = df.pop('previous_month_reviewed_by')   # 無防護
    b = df.pop('current_month_reviewed_by')    # 無防護
```

**影響**：若這兩個欄位不存在，整個後處理步驟會失敗，錯誤訊息為 `KeyError: 'previous_month_reviewed_by'`，難以定位問題。

**修復方向**：

```python
def _rearrange_reviewer_col(self, df):
    if 'previous_month_reviewed_by' not in df.columns or \
       'current_month_reviewed_by' not in df.columns:
        self.logger.warning("Reviewer columns not found, skipping rearrangement")
        return df
...
#### BUG-3：`df.rename()` 遺漏賦值

**位置**：`spt_loading.py`，`_load_raw_po_file()` 和 `_load_raw_pr_file()`

```python
if 'Project Number' in df.columns:
    df.rename(columns={'Project Number': 'Project'})  # 應加 inplace=True 或 df = df.rename(...)
```

**影響**：`Project Number` 欄位不會被重命名為 `Project`，後續步驟若依賴 `Project` 欄位會找不到。
...
#### BUG-4：`PayrollDetectionStep` 的狀態欄位動態查找不安全
...
**影響**：若 DataFrame 中沒有包含「狀態」的欄位（如資料載入失敗），此處會拋出 `IndexError: list index out of range`，且錯誤訊息不易理解。

#### BUG-5：警告使用 `print()` 而非 logger
...
**影響**：此警告不會出現在統一的日誌系統中，難以追蹤。在非互動式環境（如後台服務）中，`print()` 輸出可能被捨棄。

### 6.3 技術債務盤點

| 項目 | 嚴重程度 | 說明 |
|---|---|---|
| `SPTDataLoadingStep` 和 `SPTPRDataLoadingStep` 的重複程式碼 | 高 | 約 900 行中有 800+ 行重複，應重構為繼承 `BaseLoadingStep` |
| 4 個孤兒步驟留在程式碼庫中 | 中 | 造成混淆，應刪除或明確標記為廢棄 |
| 跨實體依賴（SPT → SPX） | 中 | 違反模組獨立性，若 SPX 重構可能影響 SPT |
| `COMMISSION_CONFIG` 和 `PAYROLL_CONFIG` 硬編碼 | 低 | 相較其他配置驅動的設計，這些常數仍在程式碼中 |
| Sub-Context 的 `processing_date` 傳遞問題 | 低 | COMBINED 模式可能使用錯誤的處理日期進行狀態判斷 |
...
目前 SPT 模組中存在兩種截然不同的載入架構：

| 載入步驟 | 繼承自 | 並發方式 | 資源管理 |
|---|---|---|---|
| `SPTDataLoadingStep` | `PipelineStep` | `asyncio.gather()` | `DataSourcePool` + `finally` |
| `SPTPRDataLoadingStep` | `PipelineStep` | `asyncio.gather()` | `DataSourcePool` + `finally` |
...
**重構方案**：將 `SPTDataLoadingStep` 和 `SPTPRDataLoadingStep` 改為繼承 `BaseLoadingStep`，只需各自實作三個抽象方法：
...
這樣可以**消除約 800 行重複程式碼**，並確保兩個步驟使用相同的並發和資源管理機制。
...
class SPTStatusStep(PipelineStep):
    """
    .. deprecated:: 2026.03
        此步驟已被 SPTERMLogicStep 取代，將在 2026.06 移除。
    """
    def __init__(self, *args, **kwargs):
        warnings.warn(
            "SPTStatusStep is deprecated. Use SPTERMLogicStep instead.",
            DeprecationWarning,
            stacklevel=2
        )
...
建立 `tasks/spt/steps/_legacy/` 目錄，將孤兒步驟移入，並更新 `__init__.py` 不再匯出這些步驟。

**方案三：直接刪除**

若確認這些步驟沒有任何外部使用（透過 `grep` 全域搜尋確認），直接刪除是最乾淨的方案。
...
| `steps/__init__.py` | 59 | 步驟匯出 |
| `steps/spt_loading.py` | ~950 | PO/PR 資料載入 |
| `steps/spt_steps.py` | 805 | 後處理 + 4 個孤兒步驟 |
...
| 編號 | 位置 | 類型 | 觸發條件 | 症狀 |
|---|---|---|---|---|
| BUG-1 | `spt_loading.py:345` | 設計錯誤 | SPT 和 SPX 的 ap_columns 配置不同 | AP 欄位使用錯誤的定義 |
| BUG-2 | `spt_steps.py:749` | KeyError 風險 | `previous_month_reviewed_by` 欄位不存在 | 後處理步驟整體失敗 |
| BUG-3 | `spt_loading.py:312,911` | 賦值遺漏 | `Project Number` 欄位存在於原始資料 | 欄位名稱未被重命名 |
| BUG-4 | `spt_evaluation_affiliate.py:554` | IndexError 風險 | DataFrame 中沒有含「狀態」的欄位 | `list index out of range` |
| BUG-5 | `pipeline_orchestrator.py:486` | 日誌不一致 | 配置中出現未知步驟名稱 | 警告訊息不進入日誌系統 |
| BUG-6 | `spt_evaluation_accountant.py:132` | 邏輯覆蓋 | `status_column` 由 `__init__` 注入後被 `execute()` 覆蓋 | Orchestrator 注入的 `status_column` 可能被忽略 |
| BUG-7 | `spt_combined_procurement_processing.py:155` | 潛在數值錯誤 | COMBINED 模式下 sub_context 未設置 `metadata.processing_date` | 採購狀態評估可能使用預設值（0）進行日期比較 |

---

*本文件由 Claude Code 自動研究生成，基於對 `accrual_bot/tasks/spt/` 目錄下 17 個 Python 檔案的深度程式碼分析。文件涵蓋模組的設計思路、實作細節、已知問題及改善建議。若模組後續有重大修改，建議更新此文件。*
