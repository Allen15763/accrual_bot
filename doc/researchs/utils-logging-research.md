...
### 4.9 `add_custom_handler()` 的潛在重複輸出問題
...
def add_custom_handler(self, name: str, handler: logging.Handler) -> None:
    with Logger._logger_lock:
        ...
        for logger in self._loggers.values():
            logger.addHandler(handler)  # ← 添加到 root AND 所有子記錄器
```

問題：
- 若將 handler 添加到 `accrual_bot`（根）
- 同時添加到 `accrual_bot.Pipeline`（子）
- 子記錄器的 `propagate=True`，事件會向上傳到根
- 根和子的 handler 都會輸出 → **同一條日誌輸出兩次**

正確做法應該只添加到根記錄器（`self._loggers['root']`），讓子記錄器透過 propagate 自動享用。
...
### 4.12 `_initialized` 標誌的非原子性問題

```python
def __init__(self):
    if self._initialized:    # ← 讀取（無鎖）
        return
    ...
    self._initialized = True  # ← 寫入（無鎖）

def cleanup(self):
    with Logger._logger_lock:
        ...
        self._initialized = False  # ← 在 _logger_lock 內寫入，但 __init__ 讀取不在鎖內
```

`_initialized` 的讀寫分屬不同鎖保護範圍（或無鎖），在理論上存在 TOCTOU（Time-of-check to time-of-use）競態條件。CPython GIL 使得整數布林值賦值是原子的，實際執行上不太可能觸發，但這在非 CPython 實作（如 PyPy、Jython）下可能是真正的問題。
...
#### 🔴 問題 1：`_supports_color()` 的 TypeError（Bug）

**位置**：`logger.py` 第 70-71 行

```python
except Exception as err:
    sys.stderr.write(err)   # ← BUG: err 是 Exception 物件，write() 需要 str
    return False
```

`sys.stderr.write()` 只接受字串，傳入 `Exception` 物件會拋出 `TypeError`。這是一個在 except 塊內拋出新例外的問題，實際上比原始例外更難除錯。

**修正**：
```python
sys.stderr.write(f"啟用終端顏色失敗: {err}\n")
```

**嚴重性**：🔴 High（在 Windows 上每次啟動都會觸發此路徑）
...
#### 🟡 問題 3：`_initialized` 標誌的非原子性讀寫

**位置**：`__init__()` 第 142-143 行 vs `cleanup()` 第 411 行

`_initialized` 的讀取在 `__init__()` 無鎖進行，寫入在 `cleanup()` 的 `_logger_lock` 內進行，不同鎖域導致 TOCTOU 競態條件（理論風險）。

**嚴重性**：🟡 Medium（CPython GIL 下不太可能觸發，但在理論模型下不安全）

#### 🟡 問題 4：`add_custom_handler()` 添加到所有子記錄器導致重複輸出

**位置**：`logger.py` 第 308-310 行

```python
for logger in self._loggers.values():
    logger.addHandler(handler)  # 添加到 root + 所有子記錄器
```

子記錄器的 `propagate=True` 會讓訊息向上傳到根，若根和子都有相同 handler，**同一條日誌會輸出兩次**。

**嚴重性**：🟡 Medium（目前 production 程式碼未呼叫 `add_custom_handler()`，暫無實際影響）
...
| # | 位置 | 問題 | 嚴重性 | 修正建議 |
|---|------|------|--------|---------|
| 1 | logger.py:71 | `sys.stderr.write(err)` TypeError | 🔴 High | 改為 `str(err)` |
| 2 | logger.py:172 | `_setup_root_logger` 無鎖直寫 dict | 🟡 Medium | 加 `with Logger._logger_lock:` |
| 3 | __init__:142 | `_initialized` 讀寫跨鎖域 | 🟡 Medium | 讀取也在鎖內進行 |
| 4 | logger.py:308 | `add_custom_handler` 重複添加 | 🟡 Medium | 只添加到 root logger |
