...
class SPTTW_PO(BasePOProcessor):
    """SPTTW公司PO處理器"""
    ...
    def mode_1(self, fileUrl: str, file_name: str, fileUrl_previwp: str, 
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式1: {file_name}")
            self.process(fileUrl, file_name, fileUrl_previwp, fileUrl_p, fileUrl_c)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式1處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_2(self, fileUrl: str, file_name: str, fileUrl_previwp: str, 
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式2: {file_name}")
            self.process(fileUrl, file_name, fileUrl_previwp, fileUrl_p)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式2處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_3(self, fileUrl: str, file_name: str, fileUrl_previwp: str, 
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式3: {file_name}")
            self.process(fileUrl, file_name, fileUrl_previwp, fileUrl_c=fileUrl_c)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式3處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_4(self, fileUrl: str, file_name: str, fileUrl_p: str, 
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式4: {file_name}")
            self.process(fileUrl, file_name, fileUrl_p=fileUrl_p, fileUrl_c=fileUrl_c)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式4處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_5(self, fileUrl: str, file_name: str, fileUrl_p: str):
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式5: {file_name}")
            self.process(fileUrl, file_name, fileUrl_p=fileUrl_p)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式5處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_6(self, fileUrl: str, file_name: str, fileUrl_previwp: str):
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式6: {file_name}")
            self.process(fileUrl, file_name, fileUrl_previwp)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式6處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_7(self, fileUrl: str, file_name: str, fileUrl_c: str):
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式7: {file_name}")
            self.process(fileUrl, file_name, fileUrl_c=fileUrl_c)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式7處理時出錯: {str(e)}", exc_info=True)
            raise
    ...
    def mode_8(self, fileUrl: str, file_name: str):
        ...
        """
        try:
            self.logger.info(f"SPTTW PO 模式8: {file_name}")
            self.process(fileUrl, file_name)
        except Exception as e:
            self.logger.error(f"SPTTW PO 模式8處理時出錯: {str(e)}", exc_info=True)
            raise


if __name__ == "__main__":
    test_processor = SPTTW_PO()
    file_name = "202501_purchase_order_20250302_105348.csv"
    file_path = r"C:\SEA\MOB PRPO re\頂一下\202501\SPT\raw\202501_purchase_order_20250302_105348.csv"
    file_path_p = r"C:\SEA\MOB PRPO re\頂一下\202501\SPT\raw\採購底稿PO 未結PO 202501 SPTTW.xlsx"
    file_path_previwp = r"C:\SEA\MOB PRPO re\頂一下\202501\SPT\raw\PO_for前期載入.xlsx"

    # 採購用: PO + 自己的底稿 + (關單)OPTIONAL; test pass
